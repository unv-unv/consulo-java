...
public class ReplaceConstructorWithBuilderDialog extends RefactoringDialog {
    ...
    protected void doAction() {
        ...
        if (myCreateBuilderClassRadioButton.isSelected()) {
            ...
            PsiClass builderClass = JavaPsiFacade.getInstance(myProject)
        ...
        invokeRefactoring(new ReplaceConstructorWithBuilderProcessor(
            getProject(),
            myConstructors,
            myParametersMap,
            className,
            packageName,
            ((DestinationFolderComboBox) myDestinationCb).selectDirectory(
                new PackageWrapper(myConstructors[0].getManager(), packageName),
                false
            ),
    ...
    protected JComponent createCenterPanel() {
        ...
        ActionListener enableDisableListener = e -> {
    ...
    private void createUIComponents() {
        ...
        ((DestinationFolderComboBox) myDestinationCb).setData(
            myProject,
            myConstructors[0].getContainingFile().getContainingDirectory(),
            myPackageTextField.getChildComponent()
        );

        myExistentClassTF = new ReferenceEditorComboWithBrowseButton(
            e -> {
        ...
        public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
            ...
            switch (columnIndex) {
                ...
                case DEFAULT_VALUE:
                    ...
                    break;
                case SKIP_SETTER:
                    data.setInsertSetter(!(Boolean) aValue);
                    break;
                default:
                    assert false;
            }
